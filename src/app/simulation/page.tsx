...
import Globe from '@/components/visualization/Earth3D';
import PlayArrow from '@mui/icons-material/PlayArrow';
import dynamic from 'next/dynamic';
import type { MapViewProps } from '@/components/MapViewLeaflet';
import Simulation2D from './2dpage';
import {
  Box,
  Container,
  Typography,
...
export default function SimulationPage() {
  const [view, setView] = useState<'regular' | '2d'>('regular');
  const handleViewChange = (_event: React.MouseEvent<HTMLElement>, newView: 'regular' | '2d' | null) => {
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
        ...
        >
          <ToggleButton value="regular" aria-label="Regular Simulation">Regular</ToggleButton>
          <ToggleButton value="2d" aria-label="2D Simulation">2D</ToggleButton>
        </ToggleButtonGroup>
      </Box>
      {view === '2d' ? (
        <Simulation2D />
        ...
        <>
          ...
          {/* ...existing regular simulation JSX... */}
          {/* Top: controls + map */}
